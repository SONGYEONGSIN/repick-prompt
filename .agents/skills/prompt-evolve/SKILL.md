...
- **승자를 라이브러리에 승격** (번역이 아니라 이동):
  ...
  7. **플러그인 버전 patch 범프 + CHANGELOG 한 줄** — 승격은 배포물(번들) 변경이므로 버전을 올리지 않으면 **기존 설치자에게 영영 전달되지 않는다**(`Codex plugin update`가 버전을 비교한다, 2026-08-04 실측). `RELEASING.md` 버전 정책 참조 — 데이터 추가는 patch다.
     ```bash
     node -e "import('./scripts/release-version.mjs').then(m=>console.log(m.bumpPatch('plugin/.Codex-plugin/plugin.json')))"
     ```
