...
- [[2026-08-19-requirement-source-traceback/DECISION|R38 — b (무출처 우선 노출형 — 블라인드 심사 36/40, auto; 부분 근거 항목의 요구/세부 분리 판정 신규 v1.36 — 요구는 근거 있으나 구체 세부는 원문에 없는 항목을 요구/세부로 쪼개 각각 판정, 구체성·바로사용가능성 결정적 우위; c(근거 강도 등급형, 27/40 최하위)는 2차 보고를 데이터 근거와 동일한 최상위 등급으로 잘못 승격 — v1.14(감의 출처는 감으로 남긴다) 발화자 정체성 표면 재현으로 강화)]]
